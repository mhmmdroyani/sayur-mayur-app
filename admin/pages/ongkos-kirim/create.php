...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($lokasi) || $biaya <= 0) {
        ...
        $stmt = mysqli_prepare($conn, "SELECT id FROM ongkos_kirim WHERE lokasi = ?");
        mysqli_stmt_bind_param($stmt, "s", $lokasi);
        mysqli_stmt_execute($stmt);
        $check = mysqli_stmt_get_result($stmt);
        
        if (mysqli_num_rows($check) > 0) {
            $error_msg = "Lokasi '$lokasi' sudah ada!";
        } else {
            $stmt = mysqli_prepare($conn, "INSERT INTO ongkos_kirim (lokasi, biaya, deskripsi, estimasi_hari) VALUES (?, ?, ?, ?)");
            mysqli_stmt_bind_param($stmt, "sdsi", $lokasi, $biaya, $deskripsi, $estimasi_hari);
            
            if (mysqli_stmt_execute($stmt)) {
                $_SESSION['success_msg'] = "Ongkos kirim berhasil ditambahkan!";
...
<head>
  ...
  <title>Tambah Ongkos Kirim - Admin SAYUR MAYUR</title>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .back-btn-header { display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: white; border-radius: 10px; text-decoration: none; font-weight: 600; border: 2px solid #22c55e; }
    ...
    .form-input, .form-textarea, .form-select { width: 100%; padding: 12px 16px; border: 2px solid #e5e7eb; border-radius: 10px; font-size: 0.95rem; font-family: inherit; background: #fafafa; color: #1e293b; }
    .form-input:focus, .form-textarea:focus, .form-select:focus { border-color: #22c55e; outline: none; box-shadow: 0 0 0 4px rgba(34, 197, 94, 0.15); background: white; }
    .form-textarea { resize: vertical; min-height: 110px; line-height: 1.6; }
    .form-helper-text { font-size: 0.85rem; color: #64748b; margin-top: 6px; }
    .form-actions { display: flex; gap: 16px; margin-top: 45px; padding-top: 35px; border-top: 2px solid #f1f5f9; }
    .btn-submit { flex: 1; padding: 16px 32px; background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: white; border: none; border-radius: 12px; font-weight: 700; font-size: 1rem; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 8px 20px rgba(34, 197, 94, 0.35); }
    .btn-submit:hover { transform: translateY(-3px); box-shadow: 0 12px 30px rgba(34, 197, 94, 0.45); }
    .btn-cancel { flex: 1; padding: 16px 32px; background: #f3f4f6; color: #1e293b; border: 2px solid #e5e7eb; border-radius: 12px; font-weight: 700; font-size: 1rem; cursor: pointer; transition: all 0.3s ease; text-decoration: none; display: flex; align-items: center; justify-content: center; }
    .btn-cancel:hover { background: #e5e7eb; border-color: #d1d5db; }
    .info-message { background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); border: 1px solid #bbf7d0; border-radius: 12px; padding: 20px; margin-bottom: 30px; display: flex; align-items: center; gap: 16px; color: #166534; }
  ...
  <div class="admin-main">
      <div class="admin-content">
        ...
        <div class="info-message">
          ...
          <p>Tentukan lokasi pengiriman dan biaya sesuai dengan area layanan Anda.</p>
        </div>
        ...
        <div style="display: flex; justify-content: center; width: 100%;">
          <div class="form-card">
            <form method="POST" class="ongkos-kirim-form">
              <div class="form-group">
                <label class="form-label">Lokasi / Kecamatan <span class="form-label-required">*</span></label>
                <input type="text" name="lokasi" class="form-input" placeholder="Contoh: Jakarta Pusat, Bandung, Bogor" required>
              </div>

              <div class="form-row">
                <div class="form-group">
                  <label class="form-label">Biaya Kirim (Rp) <span class="form-label-required">*</span></label>
                  <input type="number" name="biaya" class="form-input" placeholder="15000" step="1000" min="0" required>
                </div>

                <div class="form-group">
                  <label class="form-label">Estimasi Hari <span class="form-label-required">*</span></label>
                  <input type="number" name="estimasi_hari" class="form-input" placeholder="1" value="1" min="1" required>
                </div>
              </div>

              <div class="form-group">
                <label class="form-label">Deskripsi (Opsional)</label>
                <textarea name="deskripsi" class="form-textarea" placeholder="Deskripsi wilayah pengiriman, catatan tambahan, dll"></textarea>
              </div>

              <div class="form-actions">
                <a href="<?= route('ongkos-kirim.index') ?>" class="btn-cancel">Batal</a>
                <button type="submit" class="btn-submit">
                  <i class="bi bi-check-circle"></i> Simpan Ongkos Kirim
                </button>
              </div>
            </form>
          </div>
